//Created by Paul at Existor Ltd as part of a neural networks tutorial 1/9/2015 //See http://www.existor.com/en/news-neural-networks.html 
//Libraries needed
...
//Number of training examples and nodes in each layer. Easier to define as compiler constants here //than dynamically allocating memory at run-time.
...
int main (int argc, char **argv)
{
  ...
  {
    //this is how many times we will train the network ////////////////////////////////// Get input and output 
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("inputs", x, NUM_INPUTS+1, 1);
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("outputs", y, NUM_OUTPUTS, 1);
    }
    ////////////////////////////////// Forward pass //////////////////////////////////
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("hidden weighted sums", zhWeightedSums, NUM_HIDDEN, 1);
    }
    ...
    for (h=0; h<NUM_HIDDEN; h++)
    {
      hActivationValues[h+1] = tanhf (zhWeightedSums[h]); //apply activation function on other hidden nodes
    }

    if (debug>=2)
    {
      displayVectorOrMatrix ("hidden node activation values", hActivationValues, NUM_HIDDEN+1, 1);
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("output weighted sums", zyWeightedSums, NUM_OUTPUTS, 1);
    }

    for (sum=0, o=0; o<NUM_OUTPUTS; o++)
    {
      probabilities[o] = expf (zyWeightedSums[o]); sum += probabilities[o]; //compute exp(z) for softmax
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("softmax probabilities", probabilities, NUM_OUTPUTS, 1);
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("output error", outputErrors, NUM_OUTPUTS, 1);
    }

    for (loss=0, o=0; o<NUM_OUTPUTS; o++)
    {
      loss -= y[o] * logf (probabilities[o]); //the loss
    }

    if (debug>=1)
    {
      printf ("Iteration: %10d, loss: %10.5f\n", t+1, loss); //output the loss
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("hidden/output weights gradient", (float*)deltaWhy, NUM_HIDDEN+1, NUM_OUTPUTS);
    }
    ...
    if (debug>=2)
    {
      displayVectorOrMatrix ("back propagated error values", hActivationValues, NUM_HIDDEN+1, 1);
    }
  ...
  displayVectorOrMatrix ("predicted output", probabilities, NUM_OUTPUTS, 1); return 0;
}
